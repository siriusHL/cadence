...
import { buildTaxPackXlsx, buildAllYearsTaxPackXlsx } from '@/lib/export';
...
// `attach` is set we also bundle the tax-pack workbook — same artefact the
// Export section downloads — so the email is self-contained. `allYears` swaps
// the single-year pack for the combined multi-year workbook (every fiscal year
// with activity), matching the Export section's "All years" download.
const Body = z.object({
...
export const POST = withAuth({}, async ({ userId, tier, req }) => {
  ...
  const { to, subject, body, year, attach, allYears } = parsed.data;
  ...
  // paid export through the email path. `allYears` bundles the combined
  // multi-year workbook (no single year required); otherwise we need the year.
  let attachment: { filename: string; content: Buffer } | undefined;
  if (attach) {
    if (!allYears && year == null) return json({ error: 'year_required_for_attachment' }, 400);
    ...
    if (allYears) {
      ...
      attachment = { filename: 'tax-pack-all-years.xlsx', content: buf };
    } else {
