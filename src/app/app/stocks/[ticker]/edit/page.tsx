...
export default function EditStockPage({ params }: { params: Promise<{ ticker: string }> }) {
    ...
    return (
      <div style={{ maxWidth: 640, margin: '32px auto' }}>
        <div className="card" style={{ padding: 28, textAlign: 'center' }}>
          ...
          <div style={{ fontSize: 13, color: '#6e6e73', marginBottom: 16 }}>{error}</div>
  ...
  if (!data) {
    return (
      <div style={{ maxWidth: 640, margin: '32px auto', textAlign: 'center', color: '#86868b' }}>
        Loading…
      </div>
    );
  }
  ...
  return (
    <div style={{ maxWidth: 720, margin: '32px auto' }}>
      <div style={{ marginBottom: 18 }}>
        <Link href="/app/stocks" style={{ fontSize: 13, color: '#6e6e73', textDecoration: 'none' }}>
          ← Back to stocks
        </Link>
      </div>
      ...
      <div className="card" style={{ padding: 0, marginBottom: 16, overflow: 'hidden' }}>
        ...
        {addOpen && (
          <div style={{ padding: '14px 18px', background: 'rgba(0,0,0,0.02)', borderBottom: '1px solid rgba(0,0,0,0.06)' }}>
              ...
              return (
                <div key={tx.id} style={{
                  ...
                  <div style={{ fontSize: 13 }}>
                    <div style={{ color: '#86868b', fontSize: 11 }}>Date</div>
                    {tx.occurred_on}
                  </div>
                  <div style={{ fontSize: 13 }}>
                    <div style={{ color: '#86868b', fontSize: 11 }}>Shares</div>
                    <span className="num">{Number(tx.quantity).toLocaleString('en-IE', { maximumFractionDigits: 4 })}</span>
                  </div>
                  <div style={{ fontSize: 13 }}>
                    <div style={{ color: '#86868b', fontSize: 11 }}>Price</div>
                    <span className="num">{curSym}{Number(tx.price_local).toFixed(2)}</span>
                  </div>
                  <div style={{ fontSize: 13 }}>
                    <div style={{ color: '#86868b', fontSize: 11 }}>Cost</div>
