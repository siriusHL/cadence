...
export interface AddHoldingFormProps {
  /** Called when the holding was successfully created. Use this to close a
   *  modal, navigate away, etc. The form will already have called
   *  router.refresh() before this fires, so the caller doesn't need to. */
  onSuccess?: () => void;
}

/**
 * Self-contained add-holding form. Owns its own state, ticker autocomplete,
 * price autofill (live + historical), and submit-to-/api/holdings flow.
 *
 * Layout: stretched to its parent's width — wrap in a card or a modal as
 * needed. No outer chrome here, on purpose.
 */
export function AddHoldingForm({ onSuccess }: AddHoldingFormProps) {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    // Refresh the page that hosts us, then hand control back to the caller.
    router.refresh();
    onSuccess?.();
  }
